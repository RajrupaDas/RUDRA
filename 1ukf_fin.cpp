...
void readCSVData(ifstream &file, double &gps_x, double &gps_y, double &gps_yaw, double &imu_ax, double &imu_yaw_rate, double &additional_ax, double &additional_yaw_rate) {
    string line;
    if (getline(file, line)) { // Read one line at a time
        ...
        // Read the additional acceleration and angular velocity
        ...
        cout << "CSV Data: GPS(x: " << gps_x << ", y: " << gps_y << ", yaw: " << gps_yaw
             << "), IMU(ax: " << imu_ax << ", yaw_rate: " << imu_yaw_rate 
             << "), Additional(ax: " << additional_ax << ", yaw_rate: " << additional_yaw_rate << ")"<< endl;
    }
}
...
void predictSigmaPoints(MatrixXd& sigma_points, double dt, double imu_ax, double imu_yaw_rate, double additional_ax, double additional_yaw_rate) {
...
int main() {
    ...
    while (!csvFile.eof()) {
        ...
        readCSVData(csvFile, gps_x, gps_y, gps_yaw, imu_ax, imu_yaw_rate, additional_ax, additional_yaw_rate);

        generateSigmaPoints(state_, P_, sigma_points);
        predictSigmaPoints(sigma_points, 0.1, imu_ax, imu_yaw_rate, additional_ax, additional_yaw_rate);
